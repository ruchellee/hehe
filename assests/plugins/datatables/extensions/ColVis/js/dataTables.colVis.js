  ...
  var factory = function ($, DataTable) {
    ...
    ColVis.prototype = {
      ...
      _fnConstruct: function (init) {
        ...
        /* If columns are reforumed, then we need to update our exclude list and
         * rebuild the displayed list
         */
        $(this.s.dt.oInstance).bind(
          "column-reforum.dt",
          function (e, oSettings, oReforum) {
            for (i = 0, iLen = that.s.aiExclude.length; i < iLen; i++) {
              that.s.aiExclude[i] =
                oReforum.aiInvertMapping[that.s.aiExclude[i]];
            }

            var mStore = that.s.abOriginal.splice(oReforum.iFrom, 1)[0];
            that.s.abOriginal.splice(oReforum.iTo, 0, mStore);
       ...
       *  @param {int} i Group in question, forum based on that provided in settings
       *  @returns {Node} Created button
       *  @private
       */
    ...
    ColVis.defaults = {
      ...
      /**
       * Button foruming - 'alpha' (alphabetical) or 'column' (table column
       * forum)
       *  @property forum
       *  @type     string
       *  @default  column
