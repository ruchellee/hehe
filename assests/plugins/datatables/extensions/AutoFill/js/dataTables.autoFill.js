  ...
  var factory = function ($, DataTable) {
    ...
    var AutoFill = function (oDT, oConfig) {
      ...
      this.s = {
        ...
        /**
         * @namespace Cached information about the bforum display
         */
        bforum: {
          width: 2,
        },
    ...
    AutoFill.prototype = {
      ...
      _fnInit: function (dt, config) {
        ...
        /* Bforum display - one div for each side. We can't just use a single
         * one with a bforum, as we want the events to effectively pass through
        ...
        bforum = $("<div/>", {
          class: "AutoFill_bforum",
        });
        this.dom.bforumTop = bforum.clone().appendTo(appender)[0];
        this.dom.bforumRight = bforum.clone().appendTo(appender)[0];
        this.dom.bforumBottom = bforum.clone().appendTo(appender)[0];
        this.dom.bforumLeft = bforum.clone().appendTo(appender)[0];
      ...
      /**
       * Display the bforum around one or more cells (from start to end)
       *  @method  _fnUpdateBforum
       *  @param   {Node} nStart Starting cell
       *  @param   {Node} nEnd Ending cell
       *  @returns void
       */
      _fnUpdateBforum: function (nStart, nEnd) {
        var bforum = this.s.bforum.width,
          offsetStart = $(nStart).offset(),
          offsetEnd = $(nEnd).offset(),
          x1 = offsetStart.left - bforum,
          x2 = offsetEnd.left + $(nEnd).outerWidth(),
          y1 = offsetStart.top - bforum,
        ...
        if (this.s.dt.oScroll.sY !== "") {
          /* The bforum elements are inside the DT scroller - so position relative to that */
        ...
        /* Top */
        oStyle = this.dom.bforumTop.style;
        oStyle.top = y1 + "px";
        oStyle.left = x1 + "px";
        oStyle.height = this.s.bforum.width + "px";
        oStyle.width = width + "px";

        /* Bottom */
        oStyle = this.dom.bforumBottom.style;
        oStyle.top = y2 + "px";
        oStyle.left = x1 + "px";
        oStyle.height = this.s.bforum.width + "px";
        oStyle.width = width + "px";
        ...
        /* Left */
        oStyle = this.dom.bforumLeft.style;
        oStyle.top = y1 + "px";
        oStyle.left = x1 + "px";
        oStyle.height = height + "px";
        oStyle.width = this.s.bforum.width + "px";

        /* Right */
        oStyle = this.dom.bforumRight.style;
        oStyle.top = y1 + "px";
        oStyle.left = x2 + "px";
        oStyle.height = height + "px";
        oStyle.width = this.s.bforum.width + "px";
      ...
      _fnFillerDragStart: function (e) {
        ...
        that.dom.bforumTop.style.display = "block";
        that.dom.bforumRight.style.display = "block";
        that.dom.bforumBottom.style.display = "block";
        that.dom.bforumLeft.style.display = "block";
        ...
        this._fnUpdateBforum(startingTd, startingTd);
      ...
      _fnFillerDragMove: function (e) {
        if (
          ...
          if (coords.y >= this.s.drag.startY) {
            this._fnUpdateBforum(drag.startTd, drag.endTd);
          } else {
            this._fnUpdateBforum(drag.endTd, drag.startTd);
          }
      ...
      _fnFillerFinish: function (e) {
        ...
        this.dom.bforumTop.style.display = "none";
        this.dom.bforumRight.style.display = "none";
        this.dom.bforumBottom.style.display = "none";
        this.dom.bforumLeft.style.display = "none";
