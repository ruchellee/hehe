...
(function (factory) {
  ...
  previewCache = {
    ...
    init: function (obj) {
      ...
      previewCache.data[id] = {
        ...
        parseTemplate: function (cat, data, fname, ftype, pId, ftr, ind) {
          ...
          return obj._generatePreviewTemplate(
            cat,
            data,
            fname,
            ftype,
        ...
        footer: obj
          ...
          .replace(/\{progress}/g, obj._renderThumbProgress()),
    ...
    count: function (id, all) {
      return !!previewCache.data[id] && !!previewCache.data[id].content
        ? all
          ? previewCache.data[id].content.length
    ...
    get: function (id, i, isDisabled) {
      ...
      if (asData) {
        cat = data.previewAsData
          ? ifSet("type", config, data.previewFileType || "generic")
          : "generic";
        cap = ifSet("caption", config);
        ftr = previewCache.footer(
          id,
          i,
          isDisabled,
    ...
    footer: function (id, i, isDisabled, size) {
      ...
      var config = data.config[i],
        ...
        actions = data.isDelete
          ? data.actions(false, showDel, showZoom, showDrag, disabled, url, key)
          : "",
        footer = data.footer.replace(/\{actions}/g, actions);
      return footer
        .replace(/\{caption}/g, caption)
        .replace(/\{size}/g, data.getSize(size))
  ...
  tModal =
    ...
    '      <div class="kv-zoom-actions pull-right">{toggleheader}{fullscreen}{bforumless}{close}</div>\n' +
  ...
  defaultFileTypeSettings = {
    ...
    flash: function (vType, vName) {
      return (
        compare(vType, "application/x-shockwave-flash", true) ||
        compare(vName, /\.(swf)$/i)
      );
    },
    pdf: function (vType, vName) {
      return (
        compare(vType, "application/pdf", true) || compare(vName, /\.(pdf)$/i)
      );
    },
    object: function () {
      return true;
  ...
  getElement = function (options, param, value) {
    return isEmpty(options) || isEmpty(options[param])
      ? value
      : $(options[param]);
  };
  uniqId = function () {
  ...
  FileInput.prototype = {
    ...
    _init: function (options) {
      ...
      self.uploadFileAttr = !isEmpty($el.attr("name"))
        ? $el.attr("name")
        : "file_data";
      self.reader = null;
      self.formdata = {};
      ...
      self.isDisabled =
        self.$element.attr("disabled") || self.$element.attr("readonly");
    ...
    _parseFilePreviewIcon: function (content, fname) {
      ...
      if (content.indexOf("{previewFileIcon}") > -1) {
        return content
          .replace(/\{previewFileIconClass}/g, self.previewFileIconClass)
          .replace(/\{previewFileIcon}/g, icn);
      }
      return content;
    },
    ...
    _listenFullScreen: function (isFullScreen) {
      ...
      $btnBord = $modal && $modal.find(".btn-bforumless");
    ...
    _initClickable: function () {
      ...
      handler($zone, "click", function (e) {
        var $target = $(e.target);
        if (
          !$target.parents(".file-preview-thumbnails").length ||
          $target.parents(".file-default-preview").length
        ) {
    ...
    _uploadClick: function (e) {
      var self = this,
        ...
        $form,
        isEnabled =
          !$btn.hasClass("disabled") && isEmpty($btn.attr("disabled"));
    ...
    _submitForm: function () {
      ...
      if (
        files &&
        self.minFileCount > 0 &&
        self._getFileCount(files.length) < self.minFileCount
      ) {
        self._noFilesError({});
        return false;
      }
    ...
    _clearPreview: function () {
      ...
      $thumbs.remove();
      if (
        !self.$preview.find(".file-preview-frame").length ||
        !self.showPreview
      ) {
        self._resetUpload();
      }
      ...
      settings = {
        ...
        onSort: function (e) {
          var oldIndex = e.oldIndex,
            newIndex = e.newIndex;
          self.initialPreview = moveArray(
            self.initialPreview,
            oldIndex,
            newIndex
          );
          self.initialPreviewConfig = moveArray(
    ...
    _getModalContent: function () {
      ...
      return self
        ...
        .replace(/\{bforumless}/g, self._getZoomButton("bforumless"))
    ...
    _listenModalEvent: function (event) {
      ...
      $modal.on(event + ".bs.modal", function (e) {
        var $btnFull = $modal.find(".btn-fullscreen"),
          $btnBord = $modal.find(".btn-bforumless");
    ...
    _initPreviewActions: function () {
      ...
      self.$preview.find(".kv-file-remove").each(function () {
        ...
        config =
          isEmpty(cache.config) && isEmpty(cache.config[index])
            ? null
            : cache.config[index];
        extraData =
          isEmpty(config) || isEmpty(config.extra)
            ? deleteExtraData
            : config.extra;
    ...
    _getThumbs: function (css) {
      css = css || "";
      return this.$preview.find(
        ".file-preview-frame:not(.file-preview-initial)" + css
      );
    },
    ...
    _generatePreviewTemplate: function (
      ...
      var self = this,
        ...
        footer =
          foot || self._renderFileFooter(caption, size, config.width, isError);
      ...
      if (cat === "text" || cat === "html") {
        ...
        content = tmplt
          ...
          .replace(/\{frameClass}/g, css)
          .replace(/\{cat}/g, ftype)
          .replace(/\{footer}/g, footer)
          .replace(/\{fileindex}/g, ind)
          .replace(/\{data}/g, sText)
          .replace(/\{template}/g, cat);
      } else {
        content = tmplt
          .replace(/\{previewId}/g, previewId)
          .replace(/\{caption}/g, caption)
          .replace(/\{frameClass}/g, css)
          .replace(/\{type}/g, ftype)
          .replace(/\{fileindex}/g, ind)
          .replace(/\{width}/g, config.width)
          .replace(/\{height}/g, config.height)
          .replace(/\{footer}/g, footer)
    ...
    _previewDefault: function (file, previewId, isDisabled) {
      ...
      if (!$previewLive.length) {
        $previewLive = $(document.createElement("div"))
          .addClass("file-live-thumbs")
          .appendTo($preview);
      }
      $previewLive.append("\n" + content);
    ...
    _readFiles: function (files) {
      ...
      readFile = function (i) {
        ...
        if (!isEmpty(fileTypes) && isArray(fileTypes)) {
          ...
          if (fileCount === 0) {
            msg = self.msgInvalidFileType
              .replace("{name}", caption)
              .replace("{types}", strTypes);
            self.isError = throwError(msg, file, previewId, i);
            return;
          }
        }
        if (
          ...
          fileCount += isEmpty(chk) ? 0 : chk.length;
          if (fileCount === 0) {
            msg = self.msgInvalidFileExtension
              .replace("{name}", caption)
              .replace("{extensions}", strExt);
            self.isError = throwError(msg, file, previewId, i);
            return;
          }
        }
    ...
    _setFileDropZoneTitle: function () {
      var self = this,
        ...
        strFiles;
      if (self.isClickable) {
        strFiles = isEmpty(self.$element.attr("multiple"))
          ? self.fileSingle
          : self.filePlural;
      ...
      if (
        $zone.find(".file-preview-frame").length === 0 &&
        isEmpty(self.defaultPreviewContent)
      ) {
        $zone.prepend(
    ...
    _renderMain: function () {
      var self = this,
        ...
        css = self.isDisabled
          ...
          : self.captionClass,
        caption = self.captionTemplate.replace(
          /\{class}/g,
          css + " kv-fileinput-caption"
        );
    ...
    _renderFileFooter: function (caption, size, width, isError) {
      ...
      if (self.isUploadable) {
        out = template
          .replace(
            /\{actions}/g,
            self._renderFileActions(rem, upl, zoom, drg, false, false, false)
          )
          .replace(/\{caption}/g, caption)
          .replace(/\{size}/g, size)
          .replace(/\{width}/g, width)
    ...
    upload: function () {
      ...
      if (
        self.minFileCount > 0 &&
        self._getFileCount(totLen) < self.minFileCount
      ) {
  ...
  $.fn.fileinput.defaults = {
    ...
    previewZoomButtonIcons: {
      ...
      bforumless: '<i class="glyphicon glyphicon-resize-full"></i>',
  ...
  $.fn.fileinputLocales.en = {
    ...
    previewZoomButtonTitles: {
      ...
      bforumless: "Toggle bforumless mode",
