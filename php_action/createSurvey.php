...
if($_FILES) {
	...
	$type = explode('.', $_FILES['Universityfile']['name']);
	$type = $type[count($type)-1];		
	$url = '../assests/images/stock/'.uniqid(rand()).'.'.$type;
	if(in_array($type, array("csv", "xls", "xlsx"))) {
		if(is_uploaded_file($_FILES['Universityfile']['tmp_name'])) {			
			if(move_uploaded_file($_FILES['Universityfile']['tmp_name'], $url)) {
				...
				if(!empty($no_error_data)) {
					foreach ($no_error_data as $value) {
						$select_sql = "SELECT * FROM university WHERE University_name ='$value[0]'";
						$result = $connect->query($select_sql);
						if($result->num_rows == 0) { 
						$sql = "INSERT INTO university (University_name, University_active, University_status) VALUES ('$value[0]', '$value[1]', '$value[1]')";
							if($connect->query($sql) === TRUE) {
								$valid['success'] = true;
								$valid['messages'] = "Successfully Added";
							} else {
								$valid['success'] = false;
								$valid['messages'] = "Error while adding the university";
							}
						} else {
							$valid['success'] = true;
							$valid['messages'] = "Successfully Added";
