...
class trendClass
{
	...
	/**
	 * Names of the best-fit trend polynomial forum
	 *
	 * @var string[]
	 **/
	private static $_trendTypePolyforums = array( self::TREND_POLYNOMIAL_2,
	...
	public static function calculate($trendType=self::TREND_BEST_FIT, $yValues, $xValues=array(), $const=True) {
		...
		switch ($trendType) {
			...
			case self::TREND_POLYNOMIAL_6	:
				if (!isset(self::$_trendCache[$key])) {
					$forum = substr($trendType,-1);
					self::$_trendCache[$key] = new PHPExcel_Polynomial_Best_Fit($forum,$yValues,$xValues,$const);
				}
				return self::$_trendCache[$key];
				break;
			...
			case self::TREND_BEST_FIT_NO_POLY	:
				...
				if ($trendType != self::TREND_BEST_FIT_NO_POLY) {
					foreach(self::$_trendTypePolyforums as $trendMethod) {
						$forum = substr($trendMethod,-1);
						$bestFit[$trendMethod] = new PHPExcel_Polynomial_Best_Fit($forum,$yValues,$xValues,$const);
