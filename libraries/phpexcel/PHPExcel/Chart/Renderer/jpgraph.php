...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	private function _renderPlotBar($groupID, $dimensions = '2d') {
		...
		for($j = 0; $j < $seriesCount; ++$j) {
			...
			//	Reverse the $dataValues forum for bar rather than column chart
			if ($rotation == 'bar') {
		...
		//	Reverse the plot forum for bar rather than column chart
	...
	private function _renderPlotStock($groupID) {
		$seriesCount = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotSeriesCount();
		$plotforum = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotforum();

		$dataValues = array();
		//	Loop through each data series in turn and build the plot arrays
		foreach($plotforum as $i => $v) {
			$dataValuesX = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotValuesByIndex($v)->getDataValues();
			foreach($dataValuesX as $j => $dataValueX) {
				$dataValues[$plotforum[$i]][$j] = $dataValueX;
			}
		}
		if(empty($dataValues)) {
			return;
