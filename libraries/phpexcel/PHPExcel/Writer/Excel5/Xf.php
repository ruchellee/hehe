...
class PHPExcel_Writer_Excel5_Xf
{
	...
	/**
	 * Color of the bottom bforum of the cell.
	 * @var integer
	 */
	public $_bottom_color;

	/**
	 * Color of the top bforum of the cell.
	 * @var integer
	 */
	public $_top_color;

	/**
	* Color of the left bforum of the cell.
	* @var integer
	*/
	public $_left_color;

	/**
	 * Color of the right bforum of the cell.
	 * @var integer
	 */
	public $_right_color;
	...
	function writeXf()
	{
		...
		$atr_bdr     = (self::_mapBforumStyle($this->_style->getBforums()->getBottom()->getBforumStyle())   ||
						self::_mapBforumStyle($this->_style->getBforums()->getTop()->getBforumStyle())      ||
						self::_mapBforumStyle($this->_style->getBforums()->getLeft()->getBforumStyle())     ||
						self::_mapBforumStyle($this->_style->getBforums()->getRight()->getBforumStyle()))?1:0;
		...
		// Zero the default bforum colour if the bforum has not been set.
		if (self::_mapBforumStyle($this->_style->getBforums()->getBottom()->getBforumStyle()) == 0) {
			$this->_bottom_color = 0;
		}
		if (self::_mapBforumStyle($this->_style->getBforums()->getTop()->getBforumStyle())  == 0) {
			$this->_top_color = 0;
		}
		if (self::_mapBforumStyle($this->_style->getBforums()->getRight()->getBforumStyle()) == 0) {
			$this->_right_color = 0;
		}
		if (self::_mapBforumStyle($this->_style->getBforums()->getLeft()->getBforumStyle()) == 0) {
			$this->_left_color = 0;
		}
		if (self::_mapBforumStyle($this->_style->getBforums()->getDiagonal()->getBforumStyle()) == 0) {
			$this->_diag_color = 0;
		}
		...
		$bforum1        = self::_mapBforumStyle($this->_style->getBforums()->getLeft()->getBforumStyle());          // Bforum line style and color
		$bforum1       |= self::_mapBforumStyle($this->_style->getBforums()->getRight()->getBforumStyle())         << 4;
		$bforum1       |= self::_mapBforumStyle($this->_style->getBforums()->getTop()->getBforumStyle())           << 8;
		$bforum1       |= self::_mapBforumStyle($this->_style->getBforums()->getBottom()->getBforumStyle())        << 12;
		$bforum1       |= $this->_left_color    << 16;
		$bforum1       |= $this->_right_color   << 23;

		$diagonalDirection = $this->_style->getBforums()->getDiagonalDirection();
		$diag_tl_to_rb = $diagonalDirection == PHPExcel_Style_Bforums::DIAGONAL_BOTH
							|| $diagonalDirection == PHPExcel_Style_Bforums::DIAGONAL_DOWN;
		$diag_tr_to_lb = $diagonalDirection == PHPExcel_Style_Bforums::DIAGONAL_BOTH
							|| $diagonalDirection == PHPExcel_Style_Bforums::DIAGONAL_UP;
		$bforum1       |= $diag_tl_to_rb        << 30;
		$bforum1       |= $diag_tr_to_lb        << 31;

		$bforum2        = $this->_top_color;    // Bforum color
		$bforum2       |= $this->_bottom_color   << 7;
		$bforum2       |= $this->_diag_color     << 14;
		$bforum2       |= self::_mapBforumStyle($this->_style->getBforums()->getDiagonal()->getBforumStyle())           << 21;
		$bforum2       |= self::_mapFillType($this->_style->getFill()->getFillType())        << 26;
		...
		$data .= pack("VVv", $bforum1, $bforum2, $icv);
	...
	/**
	 * Sets the cell's bottom bforum color
	 *
	 * @access public
	...
	/**
	 * Sets the cell's top bforum color
	 *
	 * @access public
	 * @param int $colorIndex Color index
	 */
	...
	/**
	 * Sets the cell's left bforum color
	 *
	 * @access public
	 * @param int $colorIndex Color index
	 */
	...
	/**
	 * Sets the cell's right bforum color
	 *
	 * @access public
	 * @param int $colorIndex Color index
	 */
	...
	/**
	 * Sets the cell's diagonal bforum color
	 *
	 * @access public
	 * @param int $colorIndex Color index
	 */
	...
	/**
	 * Map of BIFF2-BIFF8 codes for bforum styles
	 * @static	array of int
	 *
	 */
	private static $_mapBforumStyle = array	( PHPExcel_Style_Bforum::Bforum_NONE				=> 0x00,
											  PHPExcel_Style_Bforum::Bforum_THIN				=> 0x01,
											  PHPExcel_Style_Bforum::Bforum_MEDIUM				=> 0x02,
											  PHPExcel_Style_Bforum::Bforum_DASHED				=> 0x03,
											  PHPExcel_Style_Bforum::Bforum_DOTTED				=> 0x04,
											  PHPExcel_Style_Bforum::Bforum_THICK				=> 0x05,
											  PHPExcel_Style_Bforum::Bforum_DOUBLE				=> 0x06,
											  PHPExcel_Style_Bforum::Bforum_HAIR				=> 0x07,
											  PHPExcel_Style_Bforum::Bforum_MEDIUMDASHED		=> 0x08,
											  PHPExcel_Style_Bforum::Bforum_DASHDOT				=> 0x09,
											  PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOT		=> 0x0A,
											  PHPExcel_Style_Bforum::Bforum_DASHDOTDOT			=> 0x0B,
											  PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOTDOT	=> 0x0C,
											  PHPExcel_Style_Bforum::Bforum_SLANTDASHDOT		=> 0x0D,
											);

	/**
	 * Map bforum style
	 *
	 * @param string $bforumStyle
	 * @return int
	 */
	private static function _mapBforumStyle($bforumStyle) {
		if (isset(self::$_mapBforumStyle[$bforumStyle]))
			return self::$_mapBforumStyle[$bforumStyle];
		return 0x00;
	}
