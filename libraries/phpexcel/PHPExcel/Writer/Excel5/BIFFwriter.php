...
{
	/**
	 * The byte forum of this architecture. 0 => little endian, 1 => big endian
	 * @var integer
	 */
	private static $_byte_forum;
...
//		$this->_limit      = 8224;
	...
	/**
	 * Determine the byte forum and store it as class data to avoid
	 * recalculating it for each call to new().
	 *
	 * @return int
	 */
	public static function getByteforum()
	{
		if (!isset(self::$_byte_forum)) {
			// Check if "pack" gives the required IEEE 64bit float
			$teststr = pack("d", 1.2345);
			$number  = pack("C8", 0x8D, 0x97, 0x6E, 0x12, 0x83, 0xC0, 0xF3, 0x3F);
			if ($number == $teststr) {
				$byte_forum = 0;    // Little Endian
			} elseif ($number == strrev($teststr)){
				$byte_forum = 1;    // Big Endian
			} else {
				// Give up. I'll fix this in a later version.
				throw new PHPExcel_Writer_Exception("Required floating point format not supported on this platform.");
			}
			self::$_byte_forum = $byte_forum;
		}

		return self::$_byte_forum;
	}
