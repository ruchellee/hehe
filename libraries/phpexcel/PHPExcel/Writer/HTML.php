...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	 ...
	 * Map bforum style
	 *
	 * @param	int		$bforumStyle		Sheet index
	 * @return	string
	 */
	private function _mapBforumStyle($bforumStyle) {
		switch ($bforumStyle) {
			case PHPExcel_Style_Bforum::Bforum_NONE:				return 'none';
			case PHPExcel_Style_Bforum::Bforum_DASHDOT:				return '1px dashed';
			case PHPExcel_Style_Bforum::Bforum_DASHDOTDOT:			return '1px dotted';
			case PHPExcel_Style_Bforum::Bforum_DASHED:				return '1px dashed';
			case PHPExcel_Style_Bforum::Bforum_DOTTED:				return '1px dotted';
			case PHPExcel_Style_Bforum::Bforum_DOUBLE:				return '3px double';
			case PHPExcel_Style_Bforum::Bforum_HAIR:				return '1px solid';
			case PHPExcel_Style_Bforum::Bforum_MEDIUM:				return '2px solid';
			case PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOT:		return '2px dashed';
			case PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOTDOT:	return '2px dotted';
			case PHPExcel_Style_Bforum::Bforum_MEDIUMDASHED:		return '2px dashed';
			case PHPExcel_Style_Bforum::Bforum_SLANTDASHDOT:		return '2px dashed';
			case PHPExcel_Style_Bforum::Bforum_THICK:				return '3px solid';
			case PHPExcel_Style_Bforum::Bforum_THIN:				return '1px solid';
			...
			if ($chart instanceof PHPExcel_Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					...
					if ($fp = fopen($chartFileName,"rb", 0)) {
						...
						$html .= '<div style="position: relative;">';
						$html .= '<img style="position: absolute; z-index: 1; left: ' . $chartCoordinates['xOffset'] . 'px; top: ' . $chartCoordinates['yOffset'] . 'px; width: ' . $imageDetails[0] . 'px; height: ' . $imageDetails[1] . 'px;" src="' . $imageData . '" bforum="0" />' . PHP_EOL;
						$html .= '</div>';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		// table { }
		$css['table']['bforum-collapse']  = 'collapse';
	    if (!$this->_isPdf) {
			$css['table']['page-break-after'] = 'always';
		}

		// .gridlines td { }
		$css['.gridlines td']['bforum'] = '1px dotted black';
	...
	private function _createCSSStyle(PHPExcel_Style $pStyle) {
		...
		$css = array_merge(
			...
			, $this->_createCSSStyleBforums($pStyle->getBforums())
	...
	/**
	 * Create CSS style (PHPExcel_Style_Bforums)
	 *
	 * @param	PHPExcel_Style_Bforums		$pStyle			PHPExcel_Style_Bforums
	 * @return	array
	 */
	private function _createCSSStyleBforums(PHPExcel_Style_Bforums $pStyle) {
		// Construct CSS
		$css = array();

		// Create CSS
		$css['bforum-bottom']	= $this->_createCSSStyleBforum($pStyle->getBottom());
		$css['bforum-top']		= $this->_createCSSStyleBforum($pStyle->getTop());
		$css['bforum-left']		= $this->_createCSSStyleBforum($pStyle->getLeft());
		$css['bforum-right']	= $this->_createCSSStyleBforum($pStyle->getRight());
	...
	/**
	 * Create CSS style (PHPExcel_Style_Bforum)
	 *
	 * @param	PHPExcel_Style_Bforum		$pStyle			PHPExcel_Style_Bforum
	 * @return	string
	 */
	private function _createCSSStyleBforum(PHPExcel_Style_Bforum $pStyle) {
		// Create CSS
//		$css = $this->_mapBforumStyle($pStyle->getBforumStyle()) . ' #' . $pStyle->getColor()->getRGB();
		//	Create CSS - add !important to non-none bforum styles for merged cells  
		$bforumStyle = $this->_mapBforumStyle($pStyle->getBforumStyle());  
		$css = $bforumStyle . ' #' . $pStyle->getColor()->getRGB() . (($bforumStyle == 'none') ? '' : ' !important'); 
	...
	private function _generateTableHeader($pSheet) {
		...
		if (!$this->_useInlineCss) {
			...
			$html .= '	<table bforum="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '">' . PHP_EOL;
		} else {
			$style = isset($this->_cssStyles['table']) ?
				$this->_assembleCSS($this->_cssStyles['table']) : '';

			if ($this->_isPdf && $pSheet->getShowGridlines()) {
				$html .= '	<table bforum="1" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="1" style="' . $style . '">' . PHP_EOL;
			} else {
				$html .= '	<table bforum="0" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="0" style="' . $style . '">' . PHP_EOL;
			}
		}
		...
		if (is_array($pValues)) {
			...
			foreach ($pValues as $cellAddress) {
				...
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					//	Also apply style from last cell in merge to fix bforums -
					//		relies on !important for non-none bforum declarations in _createCSSStyleBforum
