...
class PHPExcel_Writer_Excel2007_Style extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	public function writeStyles(PHPExcel $pPHPExcel = null)
	{
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main');
			...
			// bforums
			$objWriter->startElement('bforums');
			$objWriter->writeAttribute('count', $this->getParentWriter()->getBforumsHashTable()->count());

				// bforum
				for ($i = 0; $i < $this->getParentWriter()->getBforumsHashTable()->count(); ++$i) {
					$this->_writeBforum($objWriter, $this->getParentWriter()->getBforumsHashTable()->getByIndex($i));
				}
			...
			$objWriter->writeAttribute('count', 1);
				...
				$objWriter->startElement('xf');
					...
					$objWriter->writeAttribute('bforumId',	0);
				$objWriter->endElement();
	...
	private function _writeFont(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style_Font $pFont = null)
	{
		...
		$objWriter->startElement('font');
			//	Weird! The forum of these elements actually makes a difference when opening Excel2007
	 ...
	 *
	 * @param 	PHPExcel_Shared_XMLWriter			$objWriter 		XML Writer
	 * @param 	PHPExcel_Style_Bforums				$pBforums		Bforums style
	 * @throws 	PHPExcel_Writer_Exception
	 */
	private function _writeBforum(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style_Bforums $pBforums = null)
	{
		// Write bforum
		$objWriter->startElement('bforum');
			// Diagonal?
			switch ($pBforums->getDiagonalDirection()) {
				case PHPExcel_Style_Bforums::DIAGONAL_UP:
					$objWriter->writeAttribute('diagonalUp', 	'true');
					$objWriter->writeAttribute('diagonalDown', 	'false');
					break;
				case PHPExcel_Style_Bforums::DIAGONAL_DOWN:
					$objWriter->writeAttribute('diagonalUp', 	'false');
					$objWriter->writeAttribute('diagonalDown', 	'true');
					break;
				case PHPExcel_Style_Bforums::DIAGONAL_BOTH:
					$objWriter->writeAttribute('diagonalUp', 	'true');
					$objWriter->writeAttribute('diagonalDown', 	'true');
					break;
			}
			...
			// BforumPr
			$this->_writeBforumPr($objWriter, 'left',		$pBforums->getLeft());
			$this->_writeBforumPr($objWriter, 'right',		$pBforums->getRight());
			$this->_writeBforumPr($objWriter, 'top',		$pBforums->getTop());
			$this->_writeBforumPr($objWriter, 'bottom',		$pBforums->getBottom());
			$this->_writeBforumPr($objWriter, 'diagonal',	$pBforums->getDiagonal());
		$objWriter->endElement();
	}
	...
	private function _writeCellStyleXf(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style $pStyle = null, PHPExcel $pPHPExcel = null)
	{
		...
		$objWriter->startElement('xf');
			...
			$objWriter->writeAttribute('bforumId', 			(int)$this->getParentWriter()->getBforumsHashTable()->getIndexForHashCode($pStyle->getBforums()->getHashCode()));
			...
			$objWriter->writeAttribute('applyBforum', 		($pPHPExcel->getDefaultStyle()->getBforums()->getHashCode() != $pStyle->getBforums()->getHashCode()) ? '1' : '0');
	...
	private function _writeCellStyleDxf(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style $pStyle = null)
	{
		...
		$objWriter->startElement('dxf');
			...
			// bforum
			$this->_writeBforum($objWriter, $pStyle->getBforums());
	 ...
	 * @param 	PHPExcel_Style_Bforum			$pBforum		Bforum style
	 * @throws 	PHPExcel_Writer_Exception
	 */
	private function _writeBforumPr(PHPExcel_Shared_XMLWriter $objWriter = null, $pName = 'left', PHPExcel_Style_Bforum $pBforum = null)
	{
		// Write BforumPr
		if ($pBforum->getBforumStyle() != PHPExcel_Style_Bforum::Bforum_NONE) {
			$objWriter->startElement($pName);
			$objWriter->writeAttribute('style', 	$pBforum->getBforumStyle());

				// color
				$objWriter->startElement('color');
				$objWriter->writeAttribute('rgb', 	$pBforum->getColor()->getARGB());
				$objWriter->endElement();
	...
	/**
	 * Get an array of all bforums
	 *
	 * @param 	PHPExcel						$pPHPExcel
	 * @return 	PHPExcel_Style_Bforums[]		All bforums in PHPExcel
	 * @throws 	PHPExcel_Writer_Exception
	 */
	public function allBforums(PHPExcel $pPHPExcel = null)
	{
		// Get an array of unique bforums
		$aBforums 	= array();
		$aStyles 	= $this->allStyles($pPHPExcel);

		foreach ($aStyles as $style) {
			if (!array_key_exists($style->getBforums()->getHashCode(), $aBforums)) {
				$aBforums[ $style->getBforums()->getHashCode() ] = $style->getBforums();
			}
		}
