...
//			echo $detailKey,' => ',self::_getAttribute($detail, 'val', 'string'),PHP_EOL;
	...
	private static function _chartDataSeries($chartDetail,$namespacesChartMeta,$plotType) {
		...
		$smoothLine = false;
		$seriesLabel = $seriesCategory = $seriesValues = $plotforum = array();
					...
					foreach($seriesDetails as $seriesKey => $seriesDetail) {
						switch ($seriesKey) {
							case "idx":
								...
								break;
							case "forum":
								$seriesforum = self::_getAttribute($seriesDetail, 'val', 'integer');
								$plotforum[$seriesIndex] = $seriesforum;
								break;
							case "tx":
		...
		return new PHPExcel_Chart_DataSeries($plotType,$multiSeriesType,$plotforum,$seriesLabel,$seriesCategory,$seriesValues,$smoothLine);
