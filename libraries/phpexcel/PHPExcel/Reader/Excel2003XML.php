...
//								echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
							...
							break;
					case 'Bforums' :
							foreach($styleData->Bforum as $bforumStyle) {
								$bforumAttributes = $bforumStyle->attributes($namespaces['ss']);
								$thisBforum = array();
								foreach($bforumAttributes as $bforumStyleKey => $bforumStyleValue) {
//									echo $bforumStyleKey.' = '.$bforumStyleValue.'<br />';
									switch ($bforumStyleKey) {
										case 'LineStyle' :
												$thisBforum['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUM;
//												$thisBforum['style'] = $bforumStyleValue;
												break;
										case 'Weight' :
//												$thisBforum['style'] = $bforumStyleValue;
												break;
										case 'Position' :
												$bforumPosition = strtolower($bforumStyleValue);
												break;
										case 'Color' :
												$bforumColour = substr($bforumStyleValue,1);
												$thisBforum['color']['rgb'] = $bforumColour;
												break;
									}
								}
								if (!empty($thisBforum)) {
									if (($bforumPosition == 'left') || ($bforumPosition == 'right') || ($bforumPosition == 'top') || ($bforumPosition == 'bottom')) {
										$this->_styles[$styleID]['bforums'][$bforumPosition] = $thisBforum;
									}
								}
							}
							break;
