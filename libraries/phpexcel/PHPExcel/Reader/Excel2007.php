		...
		foreach ($wbRels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
					$themeforumArray = array('lt1','dk1','lt2','dk2');
					$themeforumAdditional = count($themeforumArray);
					...
					if (is_object($xmlTheme)) {
						...
						foreach ($colourScheme as $k => $xmlColour) {
							$themePos = array_search($k,$themeforumArray);
							if ($themePos === false) {
								$themePos = $themeforumAdditional++;
							}
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						foreach ($xmlStyles->cellXfs->xf as $xf) {
							...
							$style = (object) array(
								...
								"bforum" => $xmlStyles->bforums->bforum[intval($xf["bforumId"])],
...
//		}
		...
		// bforum
		if (isset($style->bforum)) {
			$diagonalUp = self::boolean((string) $style->bforum["diagonalUp"]);
			$diagonalDown = self::boolean((string) $style->bforum["diagonalDown"]);
			if (!$diagonalUp && !$diagonalDown) {
				$docStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_NONE);
			} elseif ($diagonalUp && !$diagonalDown) {
				$docStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_UP);
			} elseif (!$diagonalUp && $diagonalDown) {
				$docStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_DOWN);
			} else {
				$docStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_BOTH);
			}
			self::_readBforum($docStyle->getBforums()->getLeft(), $style->bforum->left);
			self::_readBforum($docStyle->getBforums()->getRight(), $style->bforum->right);
			self::_readBforum($docStyle->getBforums()->getTop(), $style->bforum->top);
			self::_readBforum($docStyle->getBforums()->getBottom(), $style->bforum->bottom);
			self::_readBforum($docStyle->getBforums()->getDiagonal(), $style->bforum->diagonal);
		}
	...
	private static function _readBforum($docBforum, $eleBforum) {
		if (isset($eleBforum["style"])) {
			$docBforum->setBforumStyle((string) $eleBforum["style"]);
		}
		if (isset($eleBforum->color)) {
			$docBforum->getColor()->setARGB(self::_readColor($eleBforum->color));
		}
	}
