...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function load($pFilename)
	{
		...
		// Resolve indexed colors for font, fill, and bforum colors
		...
		if (!$this->_readDataOnly) {
			...
			foreach ($this->_phpExcel->getCellXfCollection() as $objStyle) {
				...
				// bforum colors
				$top      = $objStyle->getBforums()->getTop();
				$right    = $objStyle->getBforums()->getRight();
				$bottom   = $objStyle->getBforums()->getBottom();
				$left     = $objStyle->getBforums()->getLeft();
				$diagonal = $objStyle->getBforums()->getDiagonal();

				if (isset($top->colorIndex)) {
					$bforumTopColor = self::_readColor($top->colorIndex,$this->_palette,$this->_version);
					$top->getColor()->setRGB($bforumTopColor['rgb']);
				}

				if (isset($right->colorIndex)) {
					$bforumRightColor = self::_readColor($right->colorIndex,$this->_palette,$this->_version);
					$right->getColor()->setRGB($bforumRightColor['rgb']);
				}

				if (isset($bottom->colorIndex)) {
					$bforumBottomColor = self::_readColor($bottom->colorIndex,$this->_palette,$this->_version);
					$bottom->getColor()->setRGB($bforumBottomColor['rgb']);
				}

				if (isset($left->colorIndex)) {
					$bforumLeftColor = self::_readColor($left->colorIndex,$this->_palette,$this->_version);
					$left->getColor()->setRGB($bforumLeftColor['rgb']);
				}

				if (isset($diagonal->colorIndex)) {
					$bforumDiagonalColor = self::_readColor($diagonal->colorIndex,$this->_palette,$this->_version);
					$diagonal->getColor()->setRGB($bforumDiagonalColor['rgb']);
				}
			}
		}
...
//		$this->_userDefinedProperties = $ole->getUserDefinedProperties();
	...
	private function _readSummaryInformation()
	{
		...
		// offset: 0; size: 2; must be 0xFE 0xFF (UTF-16 LE byte forum mark)
		// offset: 2; size: 2;
...
//		echo '<br />';
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell bforum lines and background area
					// bit: 3-0; mask: 0x0000000F; left style
					if ($bforumsLeftStyle = self::_mapBforumStyle((0x0000000F & self::_GetInt4d($recordData, 10)) >> 0)) {
						$objStyle->getBforums()->getLeft()->setBforumStyle($bforumsLeftStyle);
					}
					// bit: 7-4; mask: 0x000000F0; right style
					if ($bforumsRightStyle = self::_mapBforumStyle((0x000000F0 & self::_GetInt4d($recordData, 10)) >> 4)) {
						$objStyle->getBforums()->getRight()->setBforumStyle($bforumsRightStyle);
					}
					// bit: 11-8; mask: 0x00000F00; top style
					if ($bforumsTopStyle = self::_mapBforumStyle((0x00000F00 & self::_GetInt4d($recordData, 10)) >> 8)) {
						$objStyle->getBforums()->getTop()->setBforumStyle($bforumsTopStyle);
					}
					// bit: 15-12; mask: 0x0000F000; bottom style
					if ($bforumsBottomStyle = self::_mapBforumStyle((0x0000F000 & self::_GetInt4d($recordData, 10)) >> 12)) {
						$objStyle->getBforums()->getBottom()->setBforumStyle($bforumsBottomStyle);
					}
					// bit: 22-16; mask: 0x007F0000; left color
					$objStyle->getBforums()->getLeft()->colorIndex = (0x007F0000 & self::_GetInt4d($recordData, 10)) >> 16;

					// bit: 29-23; mask: 0x3F800000; right color
					$objStyle->getBforums()->getRight()->colorIndex = (0x3F800000 & self::_GetInt4d($recordData, 10)) >> 23;
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBforums()->setDiagonalDirection(PHPExcel_Style_Bforums::DIAGONAL_BOTH);
					}

				// offset: 14; size: 4;
					// bit: 6-0; mask: 0x0000007F; top color
					$objStyle->getBforums()->getTop()->colorIndex = (0x0000007F & self::_GetInt4d($recordData, 14)) >> 0;

					// bit: 13-7; mask: 0x00003F80; bottom color
					$objStyle->getBforums()->getBottom()->colorIndex = (0x00003F80 & self::_GetInt4d($recordData, 14)) >> 7;

					// bit: 20-14; mask: 0x001FC000; diagonal color
					$objStyle->getBforums()->getDiagonal()->colorIndex = (0x001FC000 & self::_GetInt4d($recordData, 14)) >> 14;

					// bit: 24-21; mask: 0x01E00000; diagonal style
					if ($bforumsDiagonalStyle = self::_mapBforumStyle((0x01E00000 & self::_GetInt4d($recordData, 14)) >> 21)) {
						$objStyle->getBforums()->getDiagonal()->setBforumStyle($bforumsDiagonalStyle);
					}
				...
				// offset: 8; size: 4; cell bforum lines and background area
				$bforumAndBackground = self::_GetInt4d($recordData, 8);

				// bit: 6-0; mask: 0x0000007F; color index for pattern color
				$objStyle->getFill()->startcolorIndex = (0x0000007F & $bforumAndBackground) >> 0;

				// bit: 13-7; mask: 0x00003F80; color index for pattern background
				$objStyle->getFill()->endcolorIndex = (0x00003F80 & $bforumAndBackground) >> 7;

				// bit: 21-16; mask: 0x003F0000; fill pattern
				$objStyle->getFill()->setFillType(self::_mapFillPattern((0x003F0000 & $bforumAndBackground) >> 16));

				// bit: 24-22; mask: 0x01C00000; bottom line style
				$objStyle->getBforums()->getBottom()->setBforumStyle(self::_mapBforumStyle((0x01C00000 & $bforumAndBackground) >> 22));

				// bit: 31-25; mask: 0xFE000000; bottom line color
				$objStyle->getBforums()->getBottom()->colorIndex = (0xFE000000 & $bforumAndBackground) >> 25;

				// offset: 12; size: 4; cell bforum lines
				$bforumLines = self::_GetInt4d($recordData, 12);

				// bit: 2-0; mask: 0x00000007; top line style
				$objStyle->getBforums()->getTop()->setBforumStyle(self::_mapBforumStyle((0x00000007 & $bforumLines) >> 0));

				// bit: 5-3; mask: 0x00000038; left line style
				$objStyle->getBforums()->getLeft()->setBforumStyle(self::_mapBforumStyle((0x00000038 & $bforumLines) >> 3));

				// bit: 8-6; mask: 0x000001C0; right line style
				$objStyle->getBforums()->getRight()->setBforumStyle(self::_mapBforumStyle((0x000001C0 & $bforumLines) >> 6));

				// bit: 15-9; mask: 0x0000FE00; top line color index
				$objStyle->getBforums()->getTop()->colorIndex = (0x0000FE00 & $bforumLines) >> 9;

				// bit: 22-16; mask: 0x007F0000; left line color index
				$objStyle->getBforums()->getLeft()->colorIndex = (0x007F0000 & $bforumLines) >> 16;

				// bit: 29-23; mask: 0x3F800000; right line color index
				$objStyle->getBforums()->getRight()->colorIndex = (0x3F800000 & $bforumLines) >> 23;
			}
				...
				switch ($extType) {
					...
					case 7:		// bforum color top
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$top = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBforums()->getTop();
					...
					case 8:		// bforum color bottom
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$bottom = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBforums()->getBottom();
					...
					case 9:		// bforum color left
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$left = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBforums()->getLeft();
					...
					case 10:		// bforum color right
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$right = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBforums()->getRight();
					...
					case 11:		// bforum color diagonal
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$diagonal = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBforums()->getDiagonal();
	...
	private function _readVcenter()
	{
		...
		if (!$this->_readDataOnly) {
			// offset: 0; size: 2; 0 = print sheet aligned at top page bforum, 1 = print sheet vertically centered
...
//			var_dump($bcBitCount);
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x61:
			...
			case 189: $function = 'DCourse'; 		$args = 3;	break;
		...
		case 0x62:
			...
			case 183: $function = 'Course';		break;
			...
			case 228: $function = 'SUMCourse';		break;
	 ...
	 * @return string
	 */
	private static function _mapBforumStyle($index)
	{
		switch ($index) {
			case 0x00: return PHPExcel_Style_Bforum::Bforum_NONE;
			case 0x01: return PHPExcel_Style_Bforum::Bforum_THIN;
			case 0x02: return PHPExcel_Style_Bforum::Bforum_MEDIUM;
			case 0x03: return PHPExcel_Style_Bforum::Bforum_DASHED;
			case 0x04: return PHPExcel_Style_Bforum::Bforum_DOTTED;
			case 0x05: return PHPExcel_Style_Bforum::Bforum_THICK;
			case 0x06: return PHPExcel_Style_Bforum::Bforum_DOUBLE;
			case 0x07: return PHPExcel_Style_Bforum::Bforum_HAIR;
			case 0x08: return PHPExcel_Style_Bforum::Bforum_MEDIUMDASHED;
			case 0x09: return PHPExcel_Style_Bforum::Bforum_DASHDOT;
			case 0x0A: return PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOT;
			case 0x0B: return PHPExcel_Style_Bforum::Bforum_DASHDOTDOT;
			case 0x0C: return PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOTDOT;
			case 0x0D: return PHPExcel_Style_Bforum::Bforum_SLANTDASHDOT;
			default:   return PHPExcel_Style_Bforum::Bforum_NONE;
		}
	}
