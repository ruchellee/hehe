...
//							echo '<br />';
							...
							if (isset($styleRegion->Style->StyleBforum)) {
								if (isset($styleRegion->Style->StyleBforum->Top)) {
									$styleArray['bforums']['top'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Top->attributes());
								}
								if (isset($styleRegion->Style->StyleBforum->Bottom)) {
									$styleArray['bforums']['bottom'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Bottom->attributes());
								}
								if (isset($styleRegion->Style->StyleBforum->Left)) {
									$styleArray['bforums']['left'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Left->attributes());
								}
								if (isset($styleRegion->Style->StyleBforum->Right)) {
									$styleArray['bforums']['right'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Right->attributes());
								}
								if ((isset($styleRegion->Style->StyleBforum->Diagonal)) && (isset($styleRegion->Style->StyleBforum->{'Rev-Diagonal'}))) {
									$styleArray['bforums']['diagonal'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Diagonal->attributes());
									$styleArray['bforums']['diagonaldirection'] = PHPExcel_Style_Bforums::DIAGONAL_BOTH;
								} elseif (isset($styleRegion->Style->StyleBforum->Diagonal)) {
									$styleArray['bforums']['diagonal'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->Diagonal->attributes());
									$styleArray['bforums']['diagonaldirection'] = PHPExcel_Style_Bforums::DIAGONAL_UP;
								} elseif (isset($styleRegion->Style->StyleBforum->{'Rev-Diagonal'})) {
									$styleArray['bforums']['diagonal'] = self::_parseBforumAttributes($styleRegion->Style->StyleBforum->{'Rev-Diagonal'}->attributes());
									$styleArray['bforums']['diagonaldirection'] = PHPExcel_Style_Bforums::DIAGONAL_DOWN;
								}
							}
...
//						echo '<br />';
	...
	private static function _parseBforumAttributes($bforumAttributes)
	{
		$styleArray = array();

		if (isset($bforumAttributes["Color"])) {
			$RGB = self::_parseGnumericColour($bforumAttributes["Color"]);
			$styleArray['color']['rgb'] = $RGB;
		}

		switch ($bforumAttributes["Style"]) {
			case '0' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_NONE;
				break;
			case '1' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_THIN;
				break;
			case '2' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUM;
				break;
			case '4' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_DASHED;
				break;
			case '5' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_THICK;
				break;
			case '6' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_DOUBLE;
				break;
			case '7' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_DOTTED;
				break;
			case '9' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_DASHDOT;
				break;
			case '10' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOT;
				break;
			case '11' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_DASHDOTDOT;
				break;
			case '12' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOTDOT;
				break;
			case '13' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUMDASHDOTDOT;
				break;
			case '3' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_SLANTDASHDOT;
				break;
			case '8' :
				$styleArray['style'] = PHPExcel_Style_Bforum::Bforum_MEDIUMDASHED;
				break;
		}
		return $styleArray;
	}
