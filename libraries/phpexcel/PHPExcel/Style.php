...
class PHPExcel_Style extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    public function __construct($isSupervisor = false, $isConditional = false)
    {
        ...
        $this->_bforums           = new PHPExcel_Style_Bforums($isSupervisor, $isConditional);
     ...
     * @param     boolean        $pAdvanced    Advanced mode for setting bforums.
    ...
    public function applyFromArray($pStyles = null, $pAdvanced = true)
    {
        if (is_array($pStyles)) {
            if ($this->_isSupervisor) {
                ...
                if ($pAdvanced && isset($pStyles['bforums'])) {

                    // 'allbforums' is a shorthand property for 'outline' and 'inside' and
                    //        it applies to components that have not been set explicitly
                    if (isset($pStyles['bforums']['allbforums'])) {
                        foreach (array('outline', 'inside') as $component) {
                            if (!isset($pStyles['bforums'][$component])) {
                                $pStyles['bforums'][$component] = $pStyles['bforums']['allbforums'];
                            }
                        }
                        unset($pStyles['bforums']['allbforums']); // not needed any more
                    }

                    // 'outline' is a shorthand property for 'top', 'right', 'bottom', 'left'
                    //        it applies to components that have not been set explicitly
                    if (isset($pStyles['bforums']['outline'])) {
                        foreach (array('top', 'right', 'bottom', 'left') as $component) {
                            if (!isset($pStyles['bforums'][$component])) {
                                $pStyles['bforums'][$component] = $pStyles['bforums']['outline'];
                            }
                        }
                        unset($pStyles['bforums']['outline']); // not needed any more
                    }

                    // 'inside' is a shorthand property for 'vertical' and 'horizontal'
                    //        it applies to components that have not been set explicitly
                    if (isset($pStyles['bforums']['inside'])) {
                        foreach (array('vertical', 'horizontal') as $component) {
                            if (!isset($pStyles['bforums'][$component])) {
                                $pStyles['bforums'][$component] = $pStyles['bforums']['inside'];
                            }
                        }
                        unset($pStyles['bforums']['inside']); // not needed any more
                    }
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        for ($y = 1; $y <= $yMax; ++$y) {
                            ...
                            unset($regionStyles['bforums']['inside']);

                            // what are the inner edges of the region when looking at the selection
                            $innerEdges = array_diff( array('top', 'right', 'bottom', 'left'), $edges );

                            // inner edges that are not touching the region should take the 'inside' bforum properties if they have been set
                            foreach ($innerEdges as $innerEdge) {
                                switch ($innerEdge) {
                                    case 'top':
                                    case 'bottom':
                                        // should pick up 'horizontal' bforum property if set
                                        if (isset($pStyles['bforums']['horizontal'])) {
                                            $regionStyles['bforums'][$innerEdge] = $pStyles['bforums']['horizontal'];
                                        } else {
                                            unset($regionStyles['bforums'][$innerEdge]);
                                        }
                                        break;
                                    case 'left':
                                    case 'right':
                                        // should pick up 'vertical' bforum property if set
                                        if (isset($pStyles['bforums']['vertical'])) {
                                            $regionStyles['bforums'][$innerEdge] = $pStyles['bforums']['vertical'];
                                        } else {
                                            unset($regionStyles['bforums'][$innerEdge]);
                                        }
                                        break;
                ...
                if (array_key_exists('bforums', $pStyles)) {
                    $this->getBforums()->applyFromArray($pStyles['bforums']);
                }
     ...
     */
    public function getBforums()
    {
        return $this->_bforums;
    }
